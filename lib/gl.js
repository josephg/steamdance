...
function GroupsWithPressure(baseGrid, engines, groups, regions, zones, currentStates) {
  ...
  baseGrid.afterWatch.on((x, y, oldV, v) => {
    if (v == null) return;
    const cm = jitutil.cellMax(v);
    for (var c = 0; c < cm; c++) {
      pendingSeedPoints.push({x, y, c});
    }
  });
  ...
  function makePressurized(seed) { // seed is a group.
    ...
    // The region is currently filled.
    if (!r0) {
      dirtySeeds.delete(seed);
      return null;
    }
  ...
  return {
    ...
    flush() {
      ...
      dirtySeeds.forEach(s => {
        ...
        const p = makePressurized(s);
        if (p != null) newPressure.push(p);
      });
