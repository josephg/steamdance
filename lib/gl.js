...
function ShuttleGeometry(shuttles) {
  ...
  verts.default = traceEdges;

  shuttles.deleteWatch.on(shuttle => verts.delete(shuttle));

  const lineTo = (dest, x, y, dir, em) => {
    const border = 1.8; // this.view.size < 5 ? 0 : (this.view.size * 0.04 + 1)|0;
    ...
    ex += border * (-dx - dy * em) / 20;
    ey += border * (-dy + dx * em) / 20;
    ...
    each((x, y, dir, v) => {
      ...
      while (!visited.has(x, y, dir)) {
        ...
        if (hasEdge(x, y, dir2=(dir+1)%4)) {
          ...
          // console.log('loopy');
          lineTo(loop, x, y, dir, 1);
          lineTo(loop, x-dy, y+dx, (dir+3)%4, 1);
...
function ShuttleBuffers(gl, shuttles, shuttleGeometry) {
  ...
  buffersForShuttle.default = shuttle => {
    const verts = shuttleGeometry.get(shuttle);
  ...
  shuttles.deleteWatch.on(shuttle => {
    ...
    if (data) gl.deleteBuffer(data.buffer);
  });
...
export default class GLRenderer {
  constructor(canvas, view) {
    ...
    const init = () => {
      this.gridShader = compileProgram(gl, glify('./grid.*.glsl'), ['proj', 'tile'], ['pos']);
      this.shuttleShader = compileProgram(gl, glify('./shuttle.*.glsl'), ['proj'], ['pos']);
  ...
  drawGrid() {
    ...
    for (let x = T(view.scrollX); x <= maxtx; x++) {
      for (let y = T(view.scrollY); y <= maxty; y++) {
        ...
        const view = this.view;
        this.setProjection(proj, TILE_SIZE * view.size, x, y);
  ...
  _drawShuttles(t) {
    ...
    this.modules.shuttles.forEach(shuttle => {
      ...
      this.setProjection(proj, view.size, sx, sy);
      gl.uniformMatrix3fv(shader.uniforms.proj, false, proj);

      const {buffer, size} = this.shuttleBuffers.get(shuttle);
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
      // index, size, type, normalized, stride, offset from the buffer
      gl.vertexAttribPointer(shader.attrs.pos, 2, gl.FLOAT, false, 8, 0);
  ...
  drawShuttles(t) {
    ...
    gl.useProgram(this.shuttleShader.program);
    ...
    if (this._drawShuttles(t)) needsRedraw = true;
