...
export default class GLRenderer {
  constructor(canvas, view) {
    ...
    const init = () => {
      ...
      this.shuttleShader = compileProgram(gl, glify('./shuttle.*.glsl'), ['proj', 'color'], ['pos']);
  ...
  _drawShuttlesOfType(t, type) {
    ...
    const numDrawn = this._drawShuttleBuffers(t, type);
    ...
    this._drawShuttleBuffers(t, type);
    return numDrawn;
  }
  ...
  drawShuttles(t) {
    ...
    gl.uniform3f(shader.uniforms.color, 0.847, 0.529, 0.972);
    this._drawShuttlesOfType(t, 'ts');

    gl.uniform3f(shader.uniforms.color, 0.58, 0.16, 0.749);
    this._drawShuttlesOfType(t, 's');
