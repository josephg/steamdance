...
class Boilerplate {
  ...
  set(x, y, bv, sv) {
    ...
    const sp = this.jit.get('shuttles', x, y) || null;
  ...
  paint() {
    ...
    line(fromtx, fromty, tx, ty, (x, y) => {
      ...
      if (this.activeTool === 'shuttle' || this.activeTool === 'thinshuttle') {
        ...
        return this.set(x, y, bv, this.activeTool);
      } else {
        return this.set(x, y, this.activeTool, null);
      }
    });
  ...
  flip(dir) {
    ...
    this._transformSelection(tw, th, dest => (x, y, v) => {
  ...
  mirror() {
    ...
    this._transformSelection(this.selection.th, this.selection.tw,
  ...
  drawShuttle(shuttle, t, isHovered) {
    ...
    shuttle.points.forEach((x, y, v) => {
      ...
      if (numLines === 0) {
        ...
        this.dctx.fillStyle = COLORS.thinshuttle;
        this.dctx.fillRect(px + size4, py + size4, size2, size2);
      }
    });

    this.pathAroundEdge(this.dctx, shuttle.pushEdges, border, {sx, sy});
    this.dctx.fillStyle = COLORS.shuttle;
    this.dctx.fill();

    if (isHovered) {
      this.pathAroundEdge(this.dctx, shuttle.pushEdges, border*2, {sx, sy});
