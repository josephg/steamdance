...
function FrameTimer(currentStates, stepWatch) {
  let time = 1;
...
export default class GLRenderer {
  ...
  addModules(jit) {
    ...
    const {baseGrid, groups, zones, currentStates, stepWatch} = modules;

    // Delete old textures so we don't leak.
    if (this.tiles) this.tiles.cleanup();

    this.frameTimer = modules.frameTimer = FrameTimer(currentStates, stepWatch);
